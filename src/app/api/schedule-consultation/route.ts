...
export async function POST(req: Request) {
  try {
    ...
    if (isOnsite) {
      const miles = await getTravelDistance(body.siteAddress);
      if (miles > TRAVEL_THRESHOLD_MILES) {
        travelSurcharge = Math.round((miles - TRAVEL_THRESHOLD_MILES) * TRAVEL_SURCHARGE_PER_MILE);
      }
    }
