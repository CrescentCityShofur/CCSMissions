      ...
      <section className="relative py-24 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Sun, title: "Solar Power", desc: "Custom solar array design and installation guidance tailored to Louisiana's climate and your energy needs." },
              { icon: Wind, title: "Wind Energy", desc: "Feasibility studies and micro-wind turbine solutions for residential and community-scale generation." },
              { icon: Droplets, title: "Water Purification", desc: "On-site water quality assessment and purification system recommendations for clean, reliable water." },
            ].map((service) => (
              <div key={service.title} className="glass-panel p-8 text-center hover:border-gold/30 transition-all duration-500 hover:-translate-y-2 group">
                <service.icon className="w-12 h-12 text-gold mx-auto mb-6 group-hover:scale-110 transition-transform" />
                <h3 className="font-cinzel text-xl text-gold mb-4 uppercase tracking-wider">{service.title}</h3>
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <div key={value.title} className="space-y-4">
                ...
                <h3 className="font-cinzel text-lg text-gold uppercase tracking-wider">{value.title}</h3>
